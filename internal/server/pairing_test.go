...
// TestHandleAPILibrarianChat_InjectsUserTokenForSession verifies that the
// per-user OPDS/MCP token of the session-authenticated user is added to the
// outbound /chat body under the `user_token` key, so the librarian can scope
// MCP calls (list_to_read, list_wishlist, list_recommendations,
// search_books unread:true) to that specific user.
func TestHandleAPILibrarianChat_InjectsUserTokenForSession(t *testing.T) {
	srv, backend, _ := newSQLiteTestServer(t, Options{Password: "pw"})
	alice, err := backend.CreateUser("Alice", "#f00", false, false, 0)
	if err != nil {
		t.Fatalf("CreateUser alice: %v", err)
	...
	if alice.Token == "" {
		t.Fatalf("Alice should have a per-user token generated by CreateUser")
	}
	...
	if got.Message != "bonjour" {
		t.Errorf("upstream message = %q, want %q", got.Message, "bonjour")
	}
	if got.UserToken != alice.Token {
		t.Errorf("upstream user_token = %q, want Alice's token %q", got.UserToken, alice.Token)
	}
}

// TestHandleAPILibrarianChat_NoUserTokenWhenAnonymous verifies that the
// outbound body is forwarded verbatim (no `user_token` key) when the request
// has no session userID and the backend has multiple users — the librarian
// will fall back to its instance token.
func TestHandleAPILibrarianChat_NoUserTokenWhenAnonymous(t *testing.T) {
	srv, backend, _ := newSQLiteTestServer(t, Options{Password: "pw"})
	// Two users registered, but the request below has no session cookie.
	if _, err := backend.CreateUser("Alice", "#f00", false, false, 0); err != nil {
		t.Fatalf("CreateUser alice: %v", err)
	}
	if _, err := backend.CreateUser("Bob", "#00f", false, false, 0); err != nil {
	...
	req.Header.Set("Content-Type", "application/json")
	req.SetBasicAuth("", "pw") // anonymous (no session userID injected)
	rr := httptest.NewRecorder()
	srv.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", rr.Code, rr.Body.String())
	...
	if gotBody != body {
		t.Errorf("upstream body = %q, want untouched %q", gotBody, body)
	}
	if strings.Contains(gotBody, "user_token") {
		t.Errorf("upstream body should not contain user_token: %q", gotBody)
	}
}

// TestHandleAPILibrarianChat_SingleUserFallback verifies that when there is no
// session userID but exactly one user is registered, that user's token is
// injected automatically — so a vanilla single-user deployment scopes chat
// calls correctly without requiring the client to log in.
func TestHandleAPILibrarianChat_SingleUserFallback(t *testing.T) {
	srv, backend, _ := newSQLiteTestServer(t, Options{Password: "pw"})
	solo, err := backend.CreateUser("Solo", "#0f0", true, false, 0)
	if err != nil {
		t.Fatalf("CreateUser solo: %v", err)
	}
	if solo.Token == "" {
		t.Fatalf("Solo should have a per-user token generated by CreateUser")
	}
	...
	req := httptest.NewRequest(http.MethodPost, "/api/ai/chat",
		strings.NewReader(`{"message":"x"}`))
	req.Header.Set("Content-Type", "application/json")
	req.SetBasicAuth("", "pw") // anonymous, but only one user exists.
	rr := httptest.NewRecorder()
	srv.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", rr.Code, rr.Body.String())
	...
	if got.UserToken != solo.Token {
		t.Errorf("upstream user_token = %q, want Solo's token %q", got.UserToken, solo.Token)
	}
}
